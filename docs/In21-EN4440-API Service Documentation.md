...
[1\. Modbus Frame Introduction	1](#modbus-frame-introduction)

[Modbus RTU Request Frame Format	1](#modbus-rtu-request-frame-format)

[Modbus RTU Response Frame Format	2](#modbus-rtu-response-frame-format)

[Modbus RTU CRC Function	2](#modbus-rtu-crc-function)

[2\. Read API	3](#read-api)
...
[Response Body	3](#response-body)

[Notes	3](#notes)

[Example	3](#example)

[3\. Write API	4](#write-api)
...
[Request Body	4](#request-body-1)

[Response Body	4](#response-body-1)

[Notes	4](#notes-1)

[Example	4](#example-1)

[4\. Modbus Data Registers	5](#modbus-data-registers)

[Description	5](#description-2)

[Notes	5](#notes-2)

[5\. Exception Codes and Meaning	6](#exception-codes-and-meaning)
...
1. # **Modbus Frame Introduction** {#modbus-frame-introduction}

## Modbus RTU Request Frame Format {#modbus-rtu-request-frame-format}
...
## Modbus RTU Response Frame Format {#modbus-rtu-response-frame-format}
...
## Modbus RTU CRC Function {#modbus-rtu-crc-function}

This is a java function that can be used to calculate the low and high bytes of a given frame.

| public int calculateCRC(byte\[\] data, int length) {         int crc \= 0xFFFF;         for (int i \= 0; i \< length; i++) {             crc ^= (data\[i\] & 0xFF);             for (int j \= 0; j \< 8; j++) {                 if ((crc & 0x0001) \!= 0\) {                     crc \>\>= 1;                     crc ^= 0xA001;                 } else {                     crc \>\>= 1;                 }             }         }         return crc & 0xFFFF; } int crc \= calculateCRC(frameWithoutCRC, length); byte low \= (byte) (crc & 0xFF); byte high \= (byte) ((crc \>\> 8\) & 0xFF); |
| :---- |
...
[http://20.15.114.131:8080/api/inverter/read](http://20.15.114.131:8080/api/inverter/read)
...
| curl \-X 'POST' \\   'http://20.15.114.131:8080/api/inverter/read' \\   \-H 'accept: \*/\*' \\   \-H 'Authorization: \<your api key\>' \\   \-H 'Content-Type: application/json' \\   \-d '{"frame": "110300000002C69B"}' |
| :---- |

3. # **Write API** {#write-api}
...
[http://20.15.114.131:8080/api/inverter/write](http://20.15.114.131:8080/api/inverter/write)
...
| curl \-X 'POST' \\   'http://20.15.114.131:8080/api/inverter/write' \\   \-H 'accept: \*/\*' \\   \-H 'Authorization: \<your api key\>' \\   \-H 'Content-Type: application/json' \\   \-d '{"frame": "1106000800100B54"}' |
| :---- |

4. # **Modbus Data Registers** {#modbus-data-registers}
...
5. # **Exception Codes and Meaning** {#exception-codes-and-meaning}
...
| 05 | Acknowledge (request accepted, processing delayed) |
...
[EN4440 API Service Keys](https://docs.google.com/spreadsheets/d/1ejMTvvgDmepvXTUiKM-69ahBjQekAfGLoSNEOSR2fwY/edit?gid=0#gid=0)
...
[http://20.15.114.131:8080/api/user/error-flag/add](http://20.15.114.131:8080/api/user/error-flag/add)
...
| {   "errorType": "EXCEPTION",   "exceptionCode": 1,   "delayMs": 0 } |
| :---- |
...
| curl \-X 'POST' \\   'http://20.15.114.131:8080/api/user/error-flag/add' \\   \-H 'accept: \*/\*' \\   \-H 'Authorization: \<your api key\>' \\   \-H 'Content-Type: application/json' \\   \-d '{   "errorType": "DELAY",   "exceptionCode": 0,   "delayMs": 10000 }'  |
| :---- |
...
[http://20.15.114.131:8080/api/inverter/error](http://20.15.114.131:8080/api/inverter/error)
...
| {   "slaveAddress": 1,   "functionCode": 1,   "errorType": "EXCEPTION",   "exceptionCode": 1,   "delayMs": 0 } |
| :---- |
...
| curl \-X 'POST' \\   'http://20.15.114.131:8080/api/inverter/error' \\   \-H 'accept: \*/\*' \\   \-H 'Content-Type: application/json' \\   \-d '{   "slaveId": 1,   "functionCode": 1,   "errorType": "EXCEPTION",   "exceptionCode": 1,   "delayMs": 0,   "dropPacket": false,   "corruptPacket": false }' |
| :---- |
