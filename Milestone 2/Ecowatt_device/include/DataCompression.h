...
#include <Arduino.h>
#include <vector>
#include "aquisition.h"  // For RegID enum
...
public:
    ...
    static String getRegisterType(RegID regId);
    ...
    struct BinaryCompressionMetrics {
        String method;              // Compression method used
        ...
        String suitabilityReason;  // Explanation of suitability
    ...
    static unsigned long measureCompressionTime(uint16_t* data, const RegID* selection, size_t count, const String& method);
    ...
    static bool validateCompressedData(const std::vector<uint8_t>& compressed, const String& method);
    
    // Check if compression method is supported
    static bool isMethodSupported(const String& method);
    
    // ==================== STATISTICS AND REPORTING ====================
    
    // Print compression statistics with both academic and traditional ratios
    static void printCompressionStats(const String& method, size_t originalSize, size_t compressedSize);
    ...
    static size_t calculateMemoryOverhead(size_t dataSize, const String& method);
    ...
    static String compressRegisterData(uint16_t* data, size_t count);
